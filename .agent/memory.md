...
- **Future-Date Only Examination Updates Engine (`js/dateUtils.js`, `tools/validate-exam-dates.js`, `tools/update-exam-dates.js` & `.github/workflows/update-exams.yml`):** Built dynamic real-time date awareness engine (`js/dateUtils.js`). Enforced strict future-date filtering (`examDate >= currentDate`), sorting closest exams first and auto-purging past dates. Created `tools/validate-exam-dates.js` integrated into GitHub Actions daily workflow. Rendered color-coded status badges (🔴 URGENT, 🟡 SOON, 🟢 UPCOMING, 🔵 FUTURE) and days remaining countdown pills on `index.html`.
